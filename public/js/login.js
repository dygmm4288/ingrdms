    ...
    loginModule.prototype.getIngrd = function(uid) {
        return new Promise((res,rej)=>{
            pool.on('acquire',(conn)=>{
                console.log("LoginModule GetIngrd Connection %d acquired",conn.threadId);
            });
            pool.on('release',(conn)=>{
                console.log("LoginModule GetIngrd Connection %d released",conn.threadId);
            });
            pool.getConnection((err,conn)=>{
                ...
                conn.query("select * from ingredient_u where user_id = ?",uid,(err,row)=>{
                    if(err) {
    ...
    loginModule.prototype.addIngrd = function(uid,ingrd) {
        ...
        return new Promise((res,rej)=>{
            ...
            pool.getConnection((err,conn)=>{
                ...
                conn.query("select ing_name from ingredient_u where ingUser_id = ? and ing_name = ? ",[uid,ingrd],(err,row)=>{
                    if(err) {
                        throw err;
                    }
                    conn.release();
            ...
            pool.getConnection((err,conn)=>{
                ...
                conn.query("insert into ingredient_u values(?,?)",[data.uid,data.ingrd],(err,row)=>{
                    if(err) {
                        throw err;
                    }
...
loginModule.prototype.setRecipe = function(uid,recipe_id)
{
    ...
    pool.getConnection((err,conn)=>{
        ...
        conn.query("insert into user_recipe values(?,?)",[uid,recipe_id],(err,row)=>{
            conn.release();
            if(err) throw err;
